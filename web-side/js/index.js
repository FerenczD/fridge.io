...
var recipeList = {};
var notified;
var key="&app_id=93f02e06&app_key=ba5504f8bdc2feb3310179d603147d3a";
$(function() {
    url = "http://127.0.0.1/fridgeio/getInventory.php";
    data = "";
    dataType = "json";
...
function createHomePageRecipes(ingredientList) {
    ...
    let recipes;
    $.ajax({
        url: 'https://api.edamam.com/search?q=' + ingredientList + key,
        dataType: "json",
        async: false,
...
function getRecipe(){
    ...
    $.ajax({
        url: 'https://api.edamam.com/search?q=' + ingredients + key,
        dataType: "json",
        async: false,
    }).then(function(response) {
        console.log(response);
...
function createRecipeList(recipes) {
    ...
    for (let i = 0; i < recipes.length; i++) {
        ...
        $('#recipeList').append("<div class=\"card recipe-card\" >\n" +
            "  <img class=\"card-img-top-recipe\" src='" + image +"' alt=\"Card image cap\">\n" +
            "  <div class=\"card-body\" style=\"margin-left:22.5%\">\n" +
            "    <h5 class=\"card-title recipe-card-title\"><b>" + title + "</b></h5>\n" +
            "    <p class=\"card-text recipe-card-text\">Calories: " + calories + "</p>\n" +
            "    <a href=\"#\" class=\"btn btn-primary recipe-card-button\" id='recp_" + i + "' onclick=\"getRecipeDetails(this.id)\" class=\"btn btn-primary recipe-card-button\">View Recipe</a>\n" +
            "  </div>\n" +
